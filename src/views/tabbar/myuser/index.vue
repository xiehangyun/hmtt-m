  ...
  <div class="my-container">
    ...
    <div class="user-info-wrap" v-if="$store.state.user">
      <div class="base-info-wrap">
        <div class="avatar-title-wrap" @click="$router.push(`/userPage/${user.id}`)">
          <van-image
            class="avatar"
            round
            fit="cover"
            :src="user.photo ? user.photo : 'https://img.yzcdn.cn/vant/cat.jpeg'"
          />
...
export default {
  ...
  watch: {},
  created () {
    this.$store.state.user && this.getUserInfo()
  },
  mounted () {
  ...
  methods: {
    ...
    async logOut () {
      await this.$dialog.confirm({ title: '退出确认', message: '您确定要退出吗？' })
